...
let
  ...
  hasCode = has "vscode";
  hasDash = has "gnomeExtensions.dash-to-dock";
  hasDesktopIcons = has "gnomeExtensions.desktop-icons-ng-ding";
  ...
  enabledExtensions = lib.optionals hasDesktopIcons [ "ding@rastersoft.com" ]
    ++ lib.optionals hasDash [ "dash-to-dock@micxgx.gmail.com" ];
...
in
{
  ...
  systemd.user.services.lab-gnome-setup = {
    ...
    path = [ pkgs.glib pkgs.gsettings-desktop-schemas pkgs.gnugrep ];
  };
}
