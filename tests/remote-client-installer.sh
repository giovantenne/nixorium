...
SCRIPT
  ...
  receipt=$(valid_plan | "$HELPER" apply)
  ...
  valid_plan | "$HELPER" apply | jq -e '.state == "accepted"' >/dev/null
  test "$(wc -l < "$fixture/systemd-calls")" -eq 1
  ! valid_plan | jq '.disk.serial = "replacement"' | "$HELPER" apply >/dev/null 2>&1
  test "$(wc -l < "$fixture/systemd-calls")" -eq 1
  ! "$HELPER" run 0123456789abcdef0123456789abcdef >/dev/null 2>&1
  "$HELPER" status 0123456789abcdef0123456789abcdef |
