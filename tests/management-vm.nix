{ nixoriumPackage }:
{
  ...
  let
    ...
    fakeUpdateGit = pkgs.writeShellScriptBin "nixorium-test-update-git" ''
      set -eu
      if [[ "$*" != "-c credential.helper= -c core.askPass= ls-remote --refs --tags --exit-code https://github.com/giovantenne/nixorium.git refs/tags/v*" ]]; then
  ...
  testScript = ''
    ...
    controller.succeed("printf '\n' >> /tmp/deployment/lab-settings.json; rm -f /tmp/nixorium-setup-resume.log; (for _ in {1..60}; do if grep -aqF 'Review and commit the generated configuration' /tmp/nixorium-setup-resume.log 2>/dev/null; then printf '\r'; sleep 2; printf '\033'; sleep 1; printf q; exit 0; fi; sleep 1; done; exit 1) | TERM=xterm timeout 70s script -qefc 'stty rows 40 cols 120; nixorium setup --repo /tmp/deployment' /tmp/nixorium-setup-resume.log")
    controller.succeed("grep -aF 'Opening the laboratory and checking setup progress' /tmp/nixorium-setup-resume.log; grep -aF 'Review and commit the generated configuration' /tmp/nixorium-setup-resume.log; grep -aF 'Git change review' /tmp/nixorium-setup-resume.log; git -C /tmp/deployment restore lab-settings.json")
