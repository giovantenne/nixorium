{ nixoriumPackage }:
{
  ...
  testScript = ''
    ...
    controller.succeed("((sleep 8; printf w; sleep 5; printf '\\t\\t'; sleep 1; printf '\\r'; sleep 1; printf '\\r'; sleep 5; printf '\\r'; sleep 5; for _ in {1..6}; do printf q; sleep 3; done) || true) | TERM=xterm timeout 50s script -qefc 'stty rows 40 cols 120; nixorium --repo /tmp/deployment' /tmp/nixorium-software-tui.log; grep -aF 'Selected software' /tmp/nixorium-software-tui.log; grep -aF '[Suggestions]' /tmp/nixorium-software-tui.log; grep -aF 'This is not the set of computers deployed today' /tmp/nixorium-software-tui.log; grep -aF 'Validated against the pinned package set' /tmp/nixorium-software-tui.log; grep -aF 'Update lab-software.json locally' /tmp/nixorium-software-tui.log; ! grep -aF 'SAVE SOFTWARE' /tmp/nixorium-software-tui.log; grep -aF 'Deploy clients to install this change' /tmp/nixorium-software-tui.log; grep -aF 'Software configuration saved' /tmp/nixorium-software-tui.log; ! grep -aF 'review Git Changes' /tmp/nixorium-software-tui.log; jq -e 'any(.packages[]; .package == \"gimp\" and .scope.kind == \"all-clients\")' /tmp/deployment/lab-software.json; ! git -C /tmp/deployment status --porcelain=v1 | grep -F 'lab-software.json'")
