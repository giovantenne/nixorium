...
in
{
  ...
  '' else ''
    ...
    controller.wait_for_unit("sshd.service")
    controller.wait_for_unit("nixorium-test-network.service")
    controller.succeed("systemctl is-active --quiet nftables.service")
    ...
    with subtest("legacy deployment lock remains exclusive inside the read-only home sandbox"):
