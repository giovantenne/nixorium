{ nixoriumPackage }:
{
  ...
  testScript = ''
    ...
    controller.succeed("su - admin -c \"(sleep 15; printf u; sleep 1; printf v2.2.0; printf '\\r'; sleep 5; printf 'UPDATE NIXORIUM TO v2.2.0\\r'; sleep 5; printf '\\003') | NIXORIUM_TEST_UPDATE_NIX_LOG=/tmp/update-tui-nix.log PATH=/tmp/fake-update-bin:\\$PATH TERM=xterm timeout 45s script -qefc 'stty rows 40 cols 120; nixorium --repo /tmp/update-deployment' /tmp/nixorium-update-tui.log\"")
    ...
    controller.succeed("su - admin -c \"(sleep 8; printf p; sleep 1; printf s; sleep 15; printf S; sleep 0.2; printf T; sleep 0.2; printf A; sleep 0.2; printf R; sleep 0.2; printf T; sleep 0.2; printf ' '; sleep 0.2; printf P; sleep 0.2; printf X; sleep 0.2; printf E; sleep 0.2; printf '\\r'; sleep 15; printf q) | TERM=xterm script -qefc 'stty rows 40 cols 120; nixorium --repo ~/nixorium-deployment' /tmp/nixorium-pxe-tui.log\"")
