{ nixoriumPackage }:
{
  ...
  testScript = ''
    ...
    controller.succeed("su - admin -c 'cd /tmp; nixorium logs --json' > /tmp/logs.json; jq -e '.operation == \"logs-list\" and .state == \"available\" and .limit == 50 and (.logs | length) >= 4 and all(.logs[]; .kind == \"deployment\" and .available and (.id | startswith(\"deploy-\")))' /tmp/logs.json")
    ...
    controller.succeed("grep -aF 'Operation logs' /tmp/nixorium-logs-tui.log; grep -aF 'Operation log detail' /tmp/nixorium-logs-tui.log; grep -aF 'Result: completed' /tmp/nixorium-logs-tui.log")
