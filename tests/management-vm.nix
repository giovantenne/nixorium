{ nixoriumPackage }:
{
  ...
  testScript = ''
    ...
    controller.succeed("nixorium --help | grep -F 'setup keys'; nixorium --help | grep -F 'git review'")
    ...
    controller.succeed("printf '\n# git-tui-review\n' >> /home/admin/nixorium-deployment/flake.nix; chown admin:users /home/admin/nixorium-deployment/flake.nix; su - admin -c \"(sleep 8; printf g; sleep 5; printf q) | TERM=xterm timeout 20s script -qefc 'nixorium --repo /home/admin/nixorium-deployment' /tmp/nixorium-git-tui.log\"; grep -aF 'Git change review' /tmp/nixorium-git-tui.log; grep -aF 'flake.nix' /tmp/nixorium-git-tui.log; grep -aF 'unstaged' /tmp/nixorium-git-tui.log; su - admin -c 'git -C ~/nixorium-deployment restore flake.nix'")
