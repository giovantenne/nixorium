...
RULES_FILE="$(mktemp)"
INTERNET_RULES_FILE="$(mktemp)"
trap 'rm -f -- "$RULES_FILE" "$INTERNET_RULES_FILE"' EXIT
nix eval --raw --file "$REPO_ROOT/tests/client-firewall-rules.nix" > "$RULES_FILE"
nix eval --raw --file "$REPO_ROOT/tests/client-internet-rules.nix" > "$INTERNET_RULES_FILE"
...
unshare --user --map-root-user --net \
  ...
  "$RULES_FILE" "$(command -v nft)" "$(command -v python3)" "$INTERNET_RULES_FILE"
