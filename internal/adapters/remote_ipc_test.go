...
func TestRemoteInstallIPCAuthenticatesPeerAndBindsResponse(t *testing.T) {
	...
	server := NewRemoteInstallIPCServer(socketPath, func(_ context.Context, request domain.RemoteInstallRequest) domain.RemoteInstallResponse {
...
func TestRemoteInstallIPCRejectsUnsafeSocketDirectory(t *testing.T) {
	...
	server := NewRemoteInstallIPCServer(filepath.Join(runtimeDirectory, "control.sock"), func(context.Context, domain.RemoteInstallRequest) domain.RemoteInstallResponse {
