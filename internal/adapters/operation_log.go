...
func openDeploymentOperation(stateRoot string, now time.Time) (*DeploymentOperation, error) {
	...
	if err != nil || !info.IsDir() {
		lock.Close()
		return nil, errors.New("deployment lock parent is not a directory")
	}
	lockInfo, err := lock.Stat()
	if err != nil || !lockInfo.Mode().IsRegular() {
		lock.Close()
		return nil, errors.New("deployment lock must be a regular file and not a symlink")
	}
	if err := lock.Chmod(0600); err != nil {
		lock.Close()
		return nil, fmt.Errorf("secure deployment lock: %w", err)
	}
	if err := syscall.Flock(lockDescriptor, syscall.LOCK_EX|syscall.LOCK_NB); err != nil {
		lock.Close()
		return nil, errors.New("another Nixorium deployment is already running")
	}
	...
	if err != nil {
		...
		lock.Close()
		return nil, fmt.Errorf("create deployment log: %w", err)
	}
	return &DeploymentOperation{Path: path, file: os.NewFile(uintptr(fileDescriptor), path), lock: lock}, nil
}
